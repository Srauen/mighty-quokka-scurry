    ...
    <Card className="glassmorphic-card flex flex-col h-full">
      <CardHeader className="p-4 border-b border-gray-700 flex flex-row items-center justify-between">
        <CardTitle className="text-lg font-semibold text-electric-blue flex items-center space-x-2">
          <Brain className="h-5 w-5 text-teal animate-pulse-orb" />
          <span>AI Insights</span>
        </CardTitle>
      </CardHeader>
      <CardContent className="flex-grow p-0">
        <Tabs defaultValue="insights" className="w-full h-full flex flex-col">
          <TabsList className="grid w-full grid-cols-3 rounded-none border-b border-gray-700 bg-transparent p-0">
            <TabsTrigger value="insights" className="rounded-none data-[state=active]:bg-gray-700 data-[state=active]:text-electric-blue data-[state=active]:shadow-none">Insights</TabsTrigger>
            <TabsTrigger value="news" className="rounded-none data-[state=active]:bg-gray-700 data-[state=active]:text-electric-blue data-[state=active]:shadow-none">News</TabsTrigger>
            <TabsTrigger value="signals" className="rounded-none data-[state=active]:bg-gray-700 data-[state=active]:text-electric-blue data-[state=active]:shadow-none">Signals</TabsTrigger>
          </TabsList>
              ...
              {insights.length === 0 ? (
                ...
                insights.map((insight) => (
                  <li key={insight.id} className="flex items-start space-x-3 text-sm">
                    ...
                    {insight.type === 'momentum' && <Signal className="h-4 w-4 text-electric-blue flex-shrink-0 mt-0.5" />}
                    ...
                    <div>
                      <p className="text-soft-white"><span className="font-bold">{insight.stock}:</span> {insight.message}</p>
              ...
              {news.length === 0 ? (
                ...
                news.map((item) => (
                  <li key={item.id} className="flex items-start space-x-3 text-sm">
                    <Newspaper className="h-4 w-4 text-gray-400 flex-shrink-0 mt-0.5" />
                    <div>
                      <p className="text-soft-white font-medium">{item.headline}</p>
          ...
          <TabsContent value="signals" className="flex-grow p-4 overflow-y-auto custom-scrollbar mt-0">
            <ul className="space-y-3">
              {signals.length === 0 ? (
                <p className="text-gray-400 text-sm">No trading signals available.</p>
              ) : (
                signals.map((signal) => (
                  <li key={signal.id} className="flex items-start space-x-3 text-sm">
                    ...
                    <div>
                      <p className="text-soft-white"><span className="font-bold">{signal.stock} ({signal.type.toUpperCase()}):</span> {signal.reason}</p>
                      <span className="text-xs text-gray-500">Strength: {signal.strength}% - {signal.timestamp}</span>
                    </div>
                  </li>
                ))
              )}
